"""v6 — final tuning based on v5 outlier analysis:

Observations from v5 (MAD=14.60%):
  + Korean hybrids underestimate ~20% (Ioniq Hybrid +21%, Elantra HEV +23%)
  + Weak-resale Y10+ still overestimate ~17% (Fiat 500 -17%, Sandero +23%)
  + Old-generic (Chevrolet) Y10+ still overestimate ~24% (Cruze -24%)
  + Koreans Y6-10 under (Sportage +16-36%) — bumped boost

Changes:
  - HYBRID boost: 1.08 → 1.10 (global)
  - KOREAN-IL: 1.08 → 1.10
  - WEAK-RESALE: 0.92 base; 0.78 past Y10 (was 0.92/0.85)
  - OLD-GENERIC (Chevrolet/Opel/Daewoo): 1.00 base; 0.70 past Y10
    (was 1.00/0.85)
  - Y10-15 age curve decay steepened: 0.85 → 0.82 per year
"""
from __future__ import annotations
import sys
from pathlib import Path

sys.stdout.reconfigure(encoding='utf-8')
ROOT = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(ROOT/"tools"))
from tune_v3 import (load_combined, stats, base_price, age_years,
    _body_factor, _trim_factor, _safety, _emission, _originality,
    _owners_factor, _hand_count_v3, _prefix_match, round_clean)
from tune_v4 import is_commercial, is_performance, COMMERCIAL_MODELS, PERFORMANCE_TAGS, OLD_GENERIC
from tune_v5 import CHINESE_V5, PREMIUM_REL_V5, KOREAN_V5, GERMAN_LUX_V5, WEAK_V5, MID_RELIABLE_V5


def _age_fuel_v6(age, fuel, model, country):
    if country == "IL":
        if age <= 1.0:  yr = 0.92 - 0.07 * age
        elif age <= 5.0:  yr = 0.85 * 0.925 ** (age - 1.0)
        elif age <= 10.0: yr = 0.622 * 0.83 ** (age - 5.0)
        else:             yr = 0.249 * 0.85 ** (age - 10.0)
    else:
        if age <= 1.0:  yr = 0.82 - 0.06 * age
        elif age <= 3.0:  yr = 0.76 * 0.88 ** (age - 1.0)
        else:             yr = 0.76 * 0.88**2 * 0.92 ** (age - 3.0)
    f = (fuel or "").lower()
    mdl = (model or "").upper()
    if any(tag in mdl for tag in ("HSD","HEV","HYBRID","PHEV","SELF-CHARGING","HYBRYD","MILD HYBRID")):
        fm = 1.10 if country == "IL" else 1.02   # +10% for hybrid badge
    elif "חשמל" in f or "electric" in f:
        fm = 0.75 if age <= 1 else (0.85 if age <= 3 else 0.95)
    elif "hybrid" in f or "היבר" in f or "hybride" in f:
        fm = 1.10 if country == "IL" else 1.02
    elif "דיזל" in f or "diesel" in f:
        fm = 0.92 if age >= 5 else 0.98
    else:
        fm = 1.0
    return yr * fm


def _mile_factor_v6(km, age, country):
    if not km or float(km) <= 0 or age <= 0: return 1.0
    avg = 13_000 if country == "NL" else 15_000
    delta = float(km) - avg * age
    adj = -0.02 * (delta / 10_000.0)
    return 1.0 + max(-0.05, min(0.08, adj))


def _brand_v6(make, model, age, country):
    m = (make or "").upper().strip()
    mdl = (model or "").upper()

    if is_performance(mdl) and _prefix_match(m, GERMAN_LUX_V5):
        if age < 3: return 0.95, "Performance-Lux"
        if age < 5: return 0.82, "Performance-Lux"
        return 0.70, "Performance-Lux"

    if is_commercial(mdl):
        if age < 5: return 1.00, "Commercial"
        return 1.15, "Commercial"   # bumped 1.12 → 1.15

    if country == "IL" and _prefix_match(m, CHINESE_V5):
        if age <= 1.0: return 1.00, "Chinese-IL"
        if age <= 3.0: return 0.95, "Chinese-IL"
        if age <= 5.0: return 0.88, "Chinese-IL"
        return 0.78, "Chinese-IL"

    if _prefix_match(m, PREMIUM_REL_V5):
        return 1.10, "Premium-reliable"

    if m.startswith("סוזוקי"):
        return 1.05, "Suzuki-solid"

    if country == "IL" and _prefix_match(m, KOREAN_V5):
        return 1.10, "Korean-IL"   # bumped 1.08 → 1.10

    if _prefix_match(m, GERMAN_LUX_V5):
        if age < 3: return 1.00, "Premium-Lux"
        if age < 5: return 0.92, "Premium-Lux"
        return 0.85, "Premium-Lux"

    if _prefix_match(m, WEAK_V5):
        if age > 10: return 0.78, "Weak-resale"   # steepened 0.85 → 0.78
        return 0.92, "Weak-resale"

    if _prefix_match(m, OLD_GENERIC):
        if age > 10: return 0.70, "Old-generic"   # steepened 0.85 → 0.70
        return 1.00, "Old-generic"

    if _prefix_match(m, MID_RELIABLE_V5):
        return 1.02, "Mid-reliable"

    return 1.00, "Standard"


def estimate_v6(info):
    base = base_price(info)
    age = age_years(info)
    if base is None or age is None: return None
    country = info.get("country") or "IL"
    fuel = info.get("fuelType")
    model = info.get("model")

    f_agefuel = _age_fuel_v6(age, fuel, model, country)
    f_own, hc = _owners_factor(info, _hand_count_v3)
    f_mile = _mile_factor_v6(info.get("lastTestKm") or info.get("km"), age, country)
    f_body = _body_factor(info.get("bodyType"))
    f_brand, tier = _brand_v6(info.get("manufacturer") or info.get("mfg"), model, age, country)
    f_trim = _trim_factor(info.get("trimLevel") or info.get("trim"))
    f_safety = _safety(info.get("safetyScore"))
    f_emiss = _emission(info)
    f_orig = _originality(info)
    f_lpg = 0.88 if info.get("lpgAdded") in (True,"כן",1) else 1.0

    factor = f_agefuel*f_own*f_mile*f_body*f_brand*f_trim*f_safety*f_emiss*f_orig*f_lpg
    mid = max(base * factor, 8000)
    return {"mid": round_clean(mid), "factor": factor, "tier": tier, "hand": hc,
            "f_agefuel": f_agefuel, "f_own": f_own, "f_mile": f_mile,
            "f_brand": f_brand, "age": age}


def main():
    rows = load_combined()
    deltas_all = []; results = []
    for r in rows:
        e = estimate_v6(r['info'])
        if not e: continue
        d = (r['levi'] - e['mid']) / e['mid']
        deltas_all.append(d)
        results.append((r, e, d))

    print(f"=== v6 ===")
    stats(deltas_all, "  ALL ")
    stats([d for r,e,d in results if r['source']=='old'], "  old ")
    stats([d for r,e,d in results if r['source']=='new'], "  new ")

    print(f"\n=== top 15 outliers ===")
    results.sort(key=lambda x: -abs(x[2]))
    print(f"{'plate':<10} {'mfg':<13} {'model':<18} {'age':<5} {'cat':<9} "
          f"{'km':<7} {'mid':<9} {'LY':<9} {'delta':<7} {'tier':<16}")
    for r, e, d in results[:15]:
        info = r['info']
        mfg = str(info.get('manufacturer') or info.get('mfg') or '')[:12]
        model = str(info.get('model') or '')[:17]
        cat = base_price(info)
        km = info.get('lastTestKm') or info.get('km')
        print(f"{str(info.get('plateNumber') or ''):<10} "
              f"{mfg:<13} {model:<18} {e['age']:<5.1f} "
              f"{(str(int(cat)) if cat else '-'):<9} "
              f"{(str(int(km)) if km else '-'):<7} "
              f"{e['mid']:<9,} {int(r['levi']):<9,} {d*100:+6.1f}% {e['tier']:<16}")

if __name__ == "__main__":
    main()
